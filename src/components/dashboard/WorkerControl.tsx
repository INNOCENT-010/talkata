...
type Worker = { provider?: string; state?: string; setup_error?: string; online: boolean; configured: boolean; worker_url_configured: boolean; busy: boolean; retry_after: number; message: string; notebook_url: string }
...
export default function WorkerControl() {
  ...
  return <section className="rounded-2xl border border-white/10 bg-[#11111a] p-4 sm:p-5 space-y-4">
    <div className="flex flex-wrap items-center justify-between gap-3">
      <div><h2 className="font-semibold text-white">Voice worker · {worker?.provider === "runpod" ? "RunPod" : "Kaggle"}</h2><p className="mt-1 text-xs text-white/45">{worker?.provider === "runpod" ? "Starts automatically when users generate audio." : "Wake your saved notebook using the existing worker address."}</p></div>
      <span className={`rounded-full px-3 py-1 text-xs ${error ? "bg-amber-500/10 text-amber-300" : worker?.online ? "bg-emerald-500/10 text-emerald-300" : "bg-white/5 text-white/60"}`}>{error ? "Status unavailable" : !worker ? "Checking…" : worker.online ? "Online" : worker.provider === "runpod" && worker.state === "sleeping" ? "Sleeping · ready for requests" : worker.busy ? "Requesting start…" : "Offline"}</span>
    </div>
    ...
    <div className="flex flex-wrap items-center gap-3">
      ...
      {worker && <a href={worker.notebook_url} target="_blank" rel="noreferrer" className="flex items-center gap-1 text-sm text-violet-300">{worker.provider === "runpod" ? "Open RunPod" : "Open Kaggle"} <ExternalLink size={14} /></a>}
    </div>
